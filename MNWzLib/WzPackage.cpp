	...
	for (int i = 0; i < nCount; ++i)
	{
		...
		if (!bTestKey)
		{
			if (nType == WzNameSpaceType::Type_Directory)
				m_mChild.insert({ sName, AllocObj(WzPackage)(m_pArchive, (WzNameSpaceType)nType, sName, uPos) });
			else if (nType == WzNameSpaceType::Type_Property)
				m_mChild.insert({ sName, AllocObj(WzNameSpaceProperty)(m_pArchive, (WzNameSpaceType)nType, sName, uPos) });
		}
	}
	m_bParsed = true;
	return true;
